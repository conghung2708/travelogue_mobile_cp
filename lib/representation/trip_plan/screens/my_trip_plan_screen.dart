  ...
  Widget _buildMasonryTripPlansList() {
    return BlocBuilder<TripPlanBloc, TripPlanState>(
      ...
      builder: (context, state) {
        if (state is GetTripPlansLoading) {
          return const Center(child: CircularProgressIndicator());
        }

        if (state is GetTripPlansError) {
          return Padding(
            padding: EdgeInsets.symmetric(horizontal: 4.w),
            child: ListView(
              children: [
        ...
        if (state is GetTripPlansSuccess) {
          final filteredTrips = state.tripPlans
              .where((t) =>
                  selectedStatus == null || t.statusEnum == selectedStatus)
              .toList();
        ...
        if (state is CreateTripPlanSuccess ||
            state is UpdateTripPlanSuccess ||
            state is UpdateTripPlanLocationsSuccess) {
          AppBloc.tripPlanBloc.add(const GetTripPlansEvent());
          return const Center(child: CircularProgressIndicator());
        }
        return Padding(
